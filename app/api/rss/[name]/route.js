...
export async function DELETE(_, { params }) {
  try {
    ...
    if (rss.state === "running") {
      throw new Error(`Delete cancelled due to RSS subscription is running, name: ${name}`);
    }
    ...
    try {
      ...
      logger.info(`RSS subscription deleted successfully, name: ${name}`, { model: "DELETE /api/rss/name" });
    ...
    logger.error(error.message, { model: "DELETE /api/rss/name" });
    return Response.json({
      code: 500,
