...
export async function testOpenAI(config) {
  try {
    ...
    logger.info(`AI server connection test successful, url: ${config.ai_api}, model: ${config.ai_model}`, { model: "testOpenAI" });
    return {
      success: true,
      message: "success",
      data: null
...
export async function parseByOpenAI(title, config) {
  ...
  try {
    ...
    if (!resultJson.title) {
      throw new Error(`Failed to parse result json, title: ${title}, json: ${result}`);
    }
    ...
    logger.error(error.message, { model: "parseByOpenAI" });
    return {
      success: false,
      message: error.message,
      data: null
