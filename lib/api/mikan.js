...
export async function getBangumiIdFromMikan(episode_url) {
  try {
    ...
    if (!homepageUrl) {
      throw new Error(`No homepage URL found at step 1, anime: ${episode_url}`);
    }
    ...
    if (!bgmId) {
      throw new Error(`No Bangumi ID found at step 2, anime: ${episode_url}`);
    }
    ...
    logger.error(error.message, { model: "getBangumiIdFromMikan" });
    return {
      success: false,
