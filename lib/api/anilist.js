...
export async function searchAnilistDetails(title_prased) {
  try {
    ...
    if (!result?.data?.Page?.media?.[0]) {
      throw new Error(`No anime details found, title: ${title_prased}`);
    }

    logger.debug(`Search anime details successfully, title: ${title_prased}`, { model: "searchAnilistDetails" });
    return {
      success: true,
      message: "success",
