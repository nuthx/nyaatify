...
export async function refreshRSS(rssId, name, url, type) {
  try {
    ...
    logger.info(`Start refreshing ${name} as ${type}`, { model: "refreshRSS" });
    ...
    for (const item of rss.items) {
      try {
        ...
        // Send notification
        await sendNotification(name, animeData);

        logger.info(`Insert anime data successfully, item: ${item.hash}, rss: ${name}`, { model: "refreshRSS" });
      } catch (error) {
        logger.error(`Failed to insert anime data, item: ${item.hash}, rss: ${name}, error: ${error.message}`, { model: "refreshRSS" });
      }
    }

    // Set state to completed, update refresh time, and increase refresh count
    await db.run("UPDATE rss SET state = 'completed', last_refreshed_at = ?, refresh_count = refresh_count + 1 WHERE id = ?", [new Date().toISOString(), rssId]);
    logger.info(`${name} has been refreshed completely`, { model: "refreshRSS" });
  } catch (error) {
    ...
    logger.error(`Failed to refresh ${name}, error: ${error.message}`, { model: "refreshRSS" });
  }
}
...
async function parseNyaaAnime(item) {
  ...
  if (item.category_id.startsWith("1_")) {
    ...
    logger.warn(`Skip parse due to category is not anime, item: ${item.hash}, category: ${item.category_id}`, { model: "parseNyaaAnime" });
  }
...
async function extractTitle(title) {
  ...
  if (config.ai_priority === "ai") {
    ...
    if (!aiResult) {
      logger.warn(`Try to extract title with Anitomy as the secondary method, title: ${title}`, { model: "extractTitle" });
...
function extractTitleWithAnitomy(title) {
  ...
  if (!parsed || !parsed.title) {
    logger.warn(`Unable to extract title with Anitomy, title: ${title}`, { model: "extractTitleWithAnitomy" });
    return null;
  }
...
export async function extractTitleWithAI(title) {
  ...
  if (!config.ai_api || !config.ai_key || !config.ai_model) {
    logger.error(`Unable to extract title with AI due to invalid config`, { model: "extractTitleWithAI" });
    return null;
  }
