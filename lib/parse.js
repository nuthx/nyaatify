...
async function extractTitle(title) {
  const db = await getDb();
  const config = await db.get("SELECT value as ai_priority FROM config WHERE key = 'ai_priority'");
...
export async function extractTitleByAI(title) {
  ...
  const config = await db.all(`
    ...
    WHERE key IN ('ai_api', 'ai_key', 'ai_model')
  `).then(rows => Object.fromEntries(rows.map(({key, value}) => [key, value])));
