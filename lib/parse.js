...
export async function refreshRSS(name) {
  try {
    ...
    await db.run("UPDATE rss SET state = 'completed', refreshed_at = ?, refresh_count = refresh_count + 1 WHERE name = ?", [new Date().toISOString(), name]);
    ...
    await db.run("UPDATE rss SET state = 'completed', refreshed_at = ? WHERE name = ?", [new Date().toISOString(), name]);
