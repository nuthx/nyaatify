    ...
    for (const item of parsedRssData.items) {
      try {
        ...
        if (!existingAnime) {
          ...
          // Insert to anime table with ignore option
          ...
          // Send notification
          await sendNotification(name, animeData, rss.refreshCount);
        } else {
...
async function parseNyaaAnime(item) {
  ...
  if (item.categoryId.startsWith("1_")) {
    ...
    logger.warn(`Skip parse due to category is not anime, anime: ${item.hash}, category: ${item.categoryId}`, { model: "parseNyaaAnime" });
  }
  ...
  return {
    ...
    titleRaw: item.title,
    titleParsed: name || "",
    titleJp: anilist?.data?.title?.native || "",
    titleCn: bangumi?.data?.name_cn || "",
    titleEn: anilist?.data?.title?.english || "",
    titleRomaji: anilist?.data?.title?.romaji || "",
    torrent: item.link,
    pubDate: new Date(item.pubDate).toISOString(),
    size: item.size,
    categoryId: item.categoryId,
    coverAnilist: anilist?.data?.coverImage?.extraLarge || "",
    coverBangumi: bangumi?.data?.images?.common || "",
    pageAnilist: anilist?.data?.id ? `https://anilist.co/anime/${anilist.data.id}` : "",
    pageBangumi: bangumi?.data?.id ? `https://bgm.tv/subject/${bangumi.data.id}` : "",
...
async function parseMikanAnime(item) {
  ...
  return {
    ...
    titleRaw: item.title,
    titleParsed: "",
    titleJp: bangumi?.data?.name || "",
    titleCn: bangumi?.data?.name_cn || "",
    titleEn: anilist?.data?.title?.english || "",
    titleRomaji: anilist?.data?.title?.romaji || "",
    torrent: item.enclosure.url,
    pubDate: new Date(item.torrent.pubDate).toISOString(),
