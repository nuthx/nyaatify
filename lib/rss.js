...
export async function refreshRSS(rssId, name, url) {
  ...
  try {
    ...
    log.info(`${name} | RSS subscription refreshed successfully`); 
  }

  catch (error) {
    log.error(`${name} | Failed to refresh RSS subscription: ${error}`);
  }
}
