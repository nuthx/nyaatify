  ...
  try {
    ...
    await dbInstance.exec(`
      ...
      CREATE TABLE IF NOT EXISTS token (
        ...
        os_platform TEXT,                      -- user os platform
        browser TEXT,                          -- browser user agent
        created_at DATETIME,                   -- created time
    ...
    await dbInstance.run(
      "INSERT OR IGNORE INTO user (username, password, salt, created_at) VALUES (?, ?, ?, ?)", 
      ["admin", hashedPassword, salt, new Date().toISOString()]
    );
