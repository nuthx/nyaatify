...
export async function getDb() {
  ...
  try {
    ...
    // Create database directory only if it doesn't exist
    try {
      await access(dbDir, constants.F_OK);
      logger.info("Attempting to open database", { model: "getDb" });
    } catch {
      await mkdir(dbDir, { recursive: true });
      logger.info("Creating database due to missing directory", { model: "getDb" });
    }

    // Open database
    // If core.db not exists, create it with open() function
    ...
    await dbInstance.exec(`
      CREATE TABLE IF NOT EXISTS anime (
        ...
        category_id TEXT,                      -- category (for nyaa rss only)
    ...
    logger.info("Database opened successfully", { model: "getDb" });
    return dbInstance;
  } catch (error) {
