...
输出必须包含以下四个键值对，若某个键没有内容则其值为空，若四个键值对都为空也要返回四个键名。不可编造或翻译:
- "titleJp"
- "titleCn"
- "titleEn"
- "titleRomaji"
...
export async function extractTitle(title) {
  ...
  // AI priority
  if (config.parserPriority === "ai-first") {
...
async function extractTitleWithAI(title) {
  try {
    // Get result from AI
    const result = await doOpenAI(SYSTEM_PROMPT, `${USER_PROMPT}${title}`)
    if (!result.success) {
...
async function extractTitleWithAnitomy(title) {
  // Get exclusions from config
  const config = await getConfig()
  const exclusions = config.anitomyTitleExclusions ? config.anitomyTitleExclusions.split("\n") : []

  // Remove invalid content from the title to avoid parsing failure
  let cleanedTitle = title
  for (const exclusion of exclusions) {
    if (exclusion.trim()) {
      cleanedTitle = cleanedTitle.replace(exclusion.trim(), "")
    }
  }
